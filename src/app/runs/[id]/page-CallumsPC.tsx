          ...
          <div className="flex items-start justify-between gap-6 flex-wrap">
            <div>
              ...
              {run.runType === "backload" && nextIdx != null && (
                <div className="mt-2">
                  {!(progress?.completedIdx ?? []).includes(0) ? (
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {run.collectionTime
                        ...
                        : <span className="text-sm text-gray-500">Open booking</span>
                      }
                    </div>
                  ) : (
              ...
              {/* Delivery booking time — show for delivery stops only (not collection stop) */}
              {nextIdx != null && stopBookingTimes.has(nextIdx) && !(run.runType === "backload" && nextIdx === 0) && (
                <div className="mt-2 flex items-center gap-2">
                  <span className="text-lg font-bold text-amber-300">Booking {stopBookingTimes.get(nextIdx)}</span>
                </div>
              )}

              {/* Next delivery info — for backload show only after collection done; for regular always show */}
              {nextIdx != null && (run.runType !== "backload" || (progress?.completedIdx ?? []).includes(0)) && (
          ...
          {editingOrder ? (
            ...
            <div className="space-y-2 mt-4">
              {stops.map((pc, idx) => {
                ...
                // Delivery stop numbers start at 1 (skipping collection stop 0)
                      ...
                      <div>
                        ...
                        {status === "on_site" && progress?.onSiteSinceMs && (
                          <div className="text-xs text-gray-500">
                            {isCollectionStop ? "Loading for " : "On site for "}{minutesBetween(progress.onSiteSinceMs, Date.now())} mins
                          </div>
                        )}
                        {status === "completed" && run?.completedMeta?.[idx] && (
                          <div className="text-xs text-gray-500">
                            {run.completedMeta[idx].arrivedISO && (
                              <span>{isCollectionStop ? "Arrived" : "Arrived"} {new Date(run.completedMeta[idx].arrivedISO!).toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit" })}</span>
                            )}
                            {run.completedMeta[idx].arrivedISO && run.completedMeta[idx].atISO && " — "}
                            {run.completedMeta[idx].atISO ? (
                              <span>{isCollectionStop ? "Loaded & away" : "Left"} {new Date(run.completedMeta[idx].atISO).toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit" })}</span>
                            ) : run.completedMeta[idx].arrivedISO ? (
                              <span> — {isCollectionStop ? "Still loading" : "Still on site"}</span>
                            ) : null}
                          </div>
                        )}
                      </div>
